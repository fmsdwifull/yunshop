...
/* 		if(ROUTE_A!='userphotoup' and ROUTE_A!='singphotoup'){
   ...
   // 今日揭晓商品
	...
	public function addShopCart(){
		...
		$yunList = $Mcartlist['yun'];
		$generalList = $Mcartlist['general'];
		if($ShopId == 0 || $ShopNum == 0){
				...
				foreach($yunList as $key=>$val){
					if($key == $ShopId){
						if(isset($cartbs) && $cartbs=='cart'){
							$yunList[$ShopId]['num'] = $ShopNum;
						}else{
							$yunList[$ShopId]['num'] = $val['num']+$ShopNum;
						}
						$shopis = 1;
					}
					/* else{
						$yunList[$key]['num'] = $val['num'];
					} */
				}
			}else{
				$yunList =array();
				$yunList[$ShopId]['num']=$ShopNum;
				$yunList[$ShopId]['sign'] = 1;
			}
			...
			if(!empty($generalList)){
				...
				$Mcartlist['general'] = (object)array();
			}
			_setcookie('Cartlist',json_encode($Mcartlist),'');
			$cart['code']=0;   //表示添加成功
		}	
		//$cart['num']=count($Mcartlist['yun'])+count($Mcartlist['general']);    //表示现在购物车有多少条记录
		echo json_encode($cart);

	}
	
	//添加购物车
	public function g_addShopCart(){
		$ShopId = safe_replace($this->segment(4));
		$ShopNum = intval(safe_replace($this->segment(5)));
		$cartbs = safe_replace($this->segment(6));	//标识从哪里加的购物车
		...
		if($ShopId == 0 || $ShopNum == 0){
			...
			if(is_array($generalList)){
				foreach($generalList as $key=>$val){
					if($key == $ShopId){
						if(isset($cartbs) && $cartbs=='cart'){
							$generalList[$ShopId]['num'] = $ShopNum;
						}else{
							$generalList[$ShopId]['num'] = $val['num']+$ShopNum;
						}
						$shopis = 1;
					}
					/* else{
						$generalList[$key]['num'] = $val['num'];
					} */
				}
			}else{
				$generalList =array();
				$generalList[$ShopId]['num']=$ShopNum;
				$generalList[$ShopId]['sign'] = 1;
			}
	...
	public function g_delCartItem(){
	   $ShopId=safe_replace($this->segment(4));
	   $cartlist=$this->Mcartlist;
	   $generalList = $cartlist['general'];
		if($ShopId==0){
		  $cart['code']=1;   //删除失败
		}else{
			if(!empty($yunList)){
	...
	public function bangding(){
		$username=safe_replace($this->segment(4));
	    	...
	    	$password=md5(safe_replace($pwd));
	    		...
	    		$openid=_getcookie('bd_openid');
	    		$img=_getcookie('bd_headimgurl');
	    		$nickname=_getcookie('bd_nickname');
			...
			if(is_array($member)){
				...
				if($member['password']==$password){
					$this->db->Query("UPDATE `@#_member` SET `openid`='$openid',`username`='$nickname' WHERE `uid`=$member[uid]");
				...
				$userip=_get_ip();
				$time=time();
				$this->db->Query("INSERT INTO `@#_member` (`username`,`openid`,`mobile`,`password`,`user_ip`,`img`,`emailcode`,`mobilecode`,`passcode`,`time`) VALUES ('$nickname','$openid','$username','$password','$userip','$img','-1','1','-1','$time')");
				$uid=$this->db->insert_id();
				 $uinfo=$this->db->GetOne("SELECT * from `@#_member` where `uid` = '$uid'");
				 if($this->db->affected_rows()){
			     		_setcookie("uid",_encrypt($uinfo['uid']),60*60*24*7);
					_setcookie("ushell",_encrypt(md5($uinfo['uid'].$uinfo['password'].$uinfo['mobile'].$uinfo['email'])),60*60*24*7);
					$user['state']=0;
				}       
			}
			echo json_encode($user);
