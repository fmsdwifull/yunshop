...
class mobile extends base {
	...
	//了解云购，引导页
	public function ydpage(){
		$webname=$this->_cfg['web_name'];
		$title="了解云购_"._cfg("web_name");
		$key="了解云购";
		include templates("mobile/index","yd");
	}
	...
	//商品列表 5元区
	public function glist_5(){
		$webname=$this->_cfg['web_name'];
		$title="5元专区_"._cfg("web_name");
		$key="5元专区";
		$between=5;
		include templates("mobile/index","glist");
	}
	...
	//商品列表 5元区
	public function glist_10(){
		$webname=$this->_cfg['web_name'];
		$title="10元专区_"._cfg("web_name");
		$key="10元专区";
		$between=10;
		include templates("mobile/index","glist");
	}
	...
	public function glistajax(){
	    $webname=$this->_cfg['web_name'];
		$between =htmlspecialchars($this->segment(4));
		...
		if($fen1){
			$count=$this->db->GetList("select * from `@#_shoplist` where `yunjiage` = '$between' and  `scene` != '1' and `q_uid` is null and `cateid`='$fen1' $select_w");
		}else{
			$count=$this->db->GetList("select * from `@#_shoplist` where `yunjiage` = '$between' and `scene` != '1' and `q_uid` is null $select_w");
		}
		if($fen1){
			$shoplist=$this->db->GetList("select * from `@#_shoplist` where `yunjiage` = '$between' and `scene` != '1' and `q_uid` is null and `cateid`='$fen1' $select_w limit $star,$end");
		}else{
			$shoplist=$this->db->GetList("select * from `@#_shoplist` where `yunjiage` = '$between' and `scene` != '1' and `q_uid` is null $select_w limit $star,$end");
		}
		$max_renqi_qishu = 1;
		$max_renqi_qishu_id = 1;
	...
	public function item(){
		...
		$itemsc=$mysql_model->GetOne("select * from `@#_shoplist` where `id`='".$item['sid']."' LIMIT 1");
		if($itemsc['scene'] == 1) {
			$scenecode=safe_replace($this->segment(5));
			if($itemsc['code'] != $scenecode) {
	...
	function wxpage(){
		$itemid=safe_replace($this->segment(4));
		$info=$this->db->GetOne("select * from `@#_1024_wx_mate_news` WHERE `id`=$itemid");
		$guser=System::load_sys_config('weixin');
		include templates("mobile/index","wxpage");
	}
}
?>