...
/* 		if(ROUTE_A!='userphotoup' and ROUTE_A!='singphotoup'){
	...
	public function getUserOrderList(){
	   ...
	   $orderlist['listItems']=$this->db->GetList("select a.id as go_id,a.*,b.*  from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where b.q_uid='$member[uid]' and a.huode!=0 order by a.time desc limit $FIdx,$EIdx " );
		...
		foreach($orderlist['listItems'] as $key=>$val){
	        ...
	        $orderlist['listItems'][$key]['is_shaidan']=$this->db->GetOne("SELECT * FROM `@#_shaidan` WHERE `sd_userid`='$val[uid]' AND `sd_shopid`='$val[shopid]' AND `sd_qishu`='$val[shopqishu]'");
	    }
		   $orderlist['code']=0;

		}
		$orderlist['count']=count($order);
		$orderlist['dizhi']=$this->db->GetOne("SELECT * FROM `@#_member_dizhi` WHERE `uid`='$member[uid]' AND `default`='Y'");
	 // echo "<pre>";
	//print_r($orderlist['listItems']);
