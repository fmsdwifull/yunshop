...
class index extends base {
	...
	public function init(){		
		...
		//新闻公告
		$newslist=$this->db->GetList("SELECT * FROM `@#_news` ORDER BY `id` DESC LIMIT 3");
	...
	*/
	public function glist(){
		...
		$between = ($this->segment(5)) ?  ($this->segment(5)) : '1';
		...
		if($cid && $bid){
			...
			$total=$this->db->GetCount("select id from `@#_shoplist` WHERE `yunjiage` = '$between' and `scene` != '1' and `q_uid` is null and `brandid`='$bid'  and `cateid` in ($sun_id_str)");
		
		}else{
			if($bid){		
				$total=$this->db->GetCount("select id from `@#_shoplist` WHERE `yunjiage` = '$between' and `scene` != '1' and `q_uid` is null and `brandid`='$bid'");			
			}elseif($cid){			
				$sun_id_str = "'".$cid."'";
				$sun_cate = $this->db->GetList("SELECT cateid from `@#_category` where  `parentid` = '$daohang[cateid]'");
				foreach($sun_cate as $v){
				...
				$total=$this->db->GetCount("select id from `@#_shoplist` WHERE `yunjiage` = '$between' and `scene` != '1' and `q_uid` is null and `cateid` in ($sun_id_str)");
			}else{		
				$total=$this->db->GetCount("select id from `@#_shoplist` WHERE `yunjiage` = '$between' and `scene` != '1' and `q_uid` is null");
			}		
		}
		...
		if($cid && $bid){
			$shoplist=$this->db->GetPage("select * from `@#_shoplist` where `yunjiage` = '$between' and `scene` != '1' and `q_uid` is null and `brandid`='$bid' and `cateid` in ($sun_id_str) $orders",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
		}else{
			if($bid){			
				$shoplist=$this->db->GetPage("select * from `@#_shoplist` where `yunjiage` = '$between' and `scene` != '1' and `q_uid` is null and `brandid`='$bid' $orders",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
			}elseif($cid){			
				$shoplist=$this->db->GetPage("select * from `@#_shoplist` where `yunjiage` = '$between' and `scene` != '1' and `q_uid` is null and `cateid` in ($sun_id_str) $orders",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
			}else{			
				$shoplist=$this->db->GetPage("select * from `@#_shoplist` where `yunjiage` = '$between' and `scene` != '1' and `q_uid` is null $orders",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
			}
		}
	...
	public function item(){
		...
		$item=$mysql_model->GetOne("select * from `@#_shoplist` where `scene` != '1' and `id`='".$itemid."' LIMIT 1");
		...
		$sid=$item['sid'];
		$sid_code=$mysql_model->GetOne("select * from `@#_shoplist` where `scene` != '1' and `sid`='$sid' order by `id` DESC LIMIT 1,1");
	...
	public function dataserver(){
		...
		//得到云购码	
		$user_shop_codes_arr = $this->db->GetList("select goucode,cqssc from `@#_member_go_record` where `uid`= '$item[q_uid]' and `shopid` = '$itemid' and `shopqishu` = '$item[qishu]'");
		$user_shop_codes = '';
		...
		$user_shop_fmod = fmod($user_shop_time_add*100+$user_shop_codes_arr[0]['cqssc'],$item['canyurenshu']);		
	...
	//echo fmod(171304853*100+35776,153).'<br>';
	...
	public function zhuanti(){
		...
		$title=$daohang_title . "_专题_"._cfg("web_name");
		include templates("zhuanti",$zt);
	}
	...
	*/
	public function general_goods_list(){
		...
		if($cid && $bid){
			...
			$total=$this->db->GetCount("select id from `@#_shop_general` WHERE `brandid`='$bid'  and `cateid` in ($sun_id_str)");
		
		}else{
			if($bid){		
				$total=$this->db->GetCount("select id from `@#_shop_general` WHERE `brandid`='$bid'");			
			}elseif($cid){			
				$sun_id_str = "'".$cid."'";
				$sun_cate = $this->db->GetList("SELECT cateid from `@#_category` where `parentid` = '$daohang[cateid]'");
				foreach($sun_cate as $v){
				...
				$total=$this->db->GetCount("select id from `@#_shop_general` WHERE `cateid` in ($sun_id_str)");
			}else{		
				$total=$this->db->GetCount("select id from `@#_shop_general` WHERE 1");
			}		
		}
		...
		if($cid && $bid){
			$shoplist=$this->db->GetPage("select * from `@#_shop_general` where `brandid`='$bid' and `cateid` in ($sun_id_str) $orders",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
		}else{
			if($bid){			
				$shoplist=$this->db->GetPage("select * from `@#_shop_general` where `brandid`='$bid' $orders",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
			}elseif($cid){			
				$shoplist=$this->db->GetPage("select * from `@#_shop_general` where `cateid` in ($sun_id_str) $orders",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
			}else{			
				$shoplist=$this->db->GetPage("select * from `@#_shop_general` where 1 $orders",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
			}
		}	
		$this_time = time();		
		include templates("index","general_glist");

	
	}
	
	//普通商品详细
	public function general_item(){
		$mysql_model=System::load_sys_class('model');
		$itemid=abs(intval(safe_replace($this->segment(4))));	
		if(!$itemid) _message("非法操作！",WEB_PATH,3);
		$item=$mysql_model->GetOne("select * from `@#_shop_general` where `id`='".$itemid."' LIMIT 1");
		if(!$item)_message("没有这个商品！",WEB_PATH,3);
		$category=$mysql_model->GetOne("select * from `@#_category` where `cateid` = '$item[cateid]' LIMIT 1");
		
		// 规格
		if($category['property']){
			$c_property = explode('|',$category['property']);
			if($item['property']){
				$result_val = explode(',',$item['property']);
				foreach($result_val as $key => $val){
					if($val){
						$property[$key]['property'] = $c_property[$key];
						$property[$key]['val'] = explode('|',$val);
					}
				}
			}
		}
	//var_dump($property);	
		...
		include templates("index","general_item");
	}
}
