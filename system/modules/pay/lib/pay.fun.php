...
function pay_insert_shop($shop='',$type=''){
	...
	$cqssc =$tocode->cqssc;
		$ssc=$cqssc['cqssc'];
		$ssc_time=$cqssc['cqssctime'];
		$ssc_sign=$cqssc['cqsscsign'];
	...
	if($u_info){
		...
		if($q){
			...
			$m_where['id']=$u_go_info['id'];
			
			$m_data['cqssc']=$cqssc['cqssc'];
			$m_data['cqssctime']=$cqssc['cqssctime'];
			$m_data['cqsscsign']=$cqssc['cqsscsign'];
