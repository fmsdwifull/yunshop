...
<div name='con-tabv' class="con-tabv">
 ...
 <tbody>
      <tr>
        ...
        <option value="0" <?php if($menuinfo['pid']==0) echo 'selected';?>>≡ 作为一级菜单 ≡</option>
       <?php foreach($categorys AS $v) { ?>
        <option value="<?php echo $v['id'] ?>" <?php if($menuinfo['pid']==$v['id']) echo 'selected';?>><?php echo $v['name']?></option>
       <?php }?>
        </select>
        </td>
      </tr>     
      <tr>
        <th>菜单名称：</th>
        <td><input type="text" name="name" class="input-text wid140" onKeyUp="value=value.replace(/[^\a-\z\A-\Z0-9\u4E00-\u9FA5\_]/g,'')" value="<?php echo $menuinfo['name']?>">
    ...
    <select name="type" class="wid150">
        <option value="click" <?php if($menuinfo['type']=='click') echo 'selected';?>>点击</option>
        <option value="view" <?php if($menuinfo['type']=='view') echo 'selected';?>>链接</option>
        </select>
        </td>
      </tr>  
       <tr>
        <th>触发反应：</th>
        <td><input type="text" name="value" class="input-text wid140"  value="<?php echo $menuinfo['value']?>">
