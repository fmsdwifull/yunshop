...
<style>
.login_index{ position:absolute; width:630px; height:438px;left:50%; top:220px; margin-left:-315px;}
.footer{ position:absolute; bottom:10px; width:100%; text-align:center; }
.login { 
	 position:absolute; width:451px; height:300px;opacity:1;filter:alpha(opacity=100);
	box-shadow:0px 0px 0px #b4641c; left:31px; top:23px;
}
.login_header{background-color:#;width:451px; height:25px;}
.login_header span{ display:inline-block; width:112px; height:25px;float:left;}


.login_title{ height:50px; line-height:50px; font-size:22px; color:#2BB2F1; text-align:center;font-family: "微软雅黑",Arial,"宋体"}

.login_form{width:451px; height:220px;margin-top:170px;}
.login_form li{ height:30px; line-height:30px; position:relative;}
.login_form span{margin:0px; display:inline-block; width:85px;line-height:40px; padding-left:40px; text-align:left;font-size:15px; color:#fff;font-weight:bold;font-family: "微软雅黑",Arial,"宋体"}
.textinput{ border:1px solid #fff;font-size:15px; position:absolute;top:5px;width:160px; height:25px; color:#000;
...
#form_but{ width:183px; height:44px; margin-top:10px; border:0px; overflow:hidden; cursor:pointer; line-height:44px; 
		 font-size:18px; color:#FFF;font-family: "微软雅黑",Arial,"宋体";
}
...
<div class="login_index">
    <div class="login">
        <!--<div class="login_header" style="border:1px solid #f00">
            <span class="login_header_span1">1</span>
        ...
        <!--<div class="login_title">欢迎登陆</div></BR>-->
        <div class="login_form">
        <ul>
        <form action="#" method="post" id="form">
        <li><span>管理账号:</span><input type="text" id="input-u" name="username" style="color:#2BB2F1;" class="textinput" value="登录名" /></li></BR>
        <li><span>管理密码::</span><input type="password" id="input-p" name="password" style="color:#0B6121;" class="textinput"  value="请输入密码" /></li></BR>
		<?php if(_cfg("web_off")){ ?>
        <li><span>验证码:</span><input type="text" id="input-c" name="code" style="color:#2BB2F1;width:60px;text-transform:uppercase;" class="textinput" value="code" />
        <img id="checkcode" src="<?php echo WEB_PATH; ?>/api/checkcode/image/80_27/"/>
        </li></BR>
		<?php } ?>
        <li style="margin-top:20px;"><span></span><input type="button" id="form_but"  value="马上登录" /></li>
        </form>
        </ul>
        </div>
    </div><!--login end-->
