	...
	public function addcate(){	
		...
		if(isset($_POST['info'])){	
			...
			switch($catetype){
				...
				$info['property']=htmlspecialchars($_POST['info']['property']);
...
echo 2;die;						
				...
				$sql="INSERT INTO `@#_category` 
				 (`parentid`, `channel`,`model`,`name`, `catdir`, `url`, `info`, `order`,`property`) 
				  VALUES 
				 ('$info[parentid]','0', '$info[modelid]','$info[name]', '$info[catdir]', '','$setting','1','$info[property]')";
	var_dump($sql);die;	
				$this->db->Query($sql);			
