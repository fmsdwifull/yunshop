...
class content extends admin {
	...
	public function news_add(){
		if(isset($_POST['dosubmit'])){
			...
			$content = editor_safe_replace(stripslashes($_POST['content']));
			$description = htmlspecialchars($_POST['description']);
			if(!$description && $_POST['sub_text_des']=='off'){
				$description=mb_substr($content,0,$_POST['sub_text_len']);
			}			
			$author = isset($_POST['zuoze']) ? htmlspecialchars($_POST['zuoze']) : '';	
			$thumb = isset($_POST['thumb']) ? htmlspecialchars($_POST['thumb']) : '';						
			$picarr = isset($_POST['uppicarr']) ? serialize($_POST['uppicarr']) : serialize(array());			
			...
			if(empty($title)){_message("标题不能为空");}
			$sql="INSERT INTO `@#_news` (`author`, `title`,`title_style`, `thumb`, `picarr`,`description`, `content`, `hit`, `order`, `posttime`) VALUES ('$author', '$title','$title_style', '$thumb', '$picarr','$description', '$content', '$hit', '1', '$posttime')";
			if($this->db->Query($sql)){
				_message('新闻添加成功',WEB_PATH.'/admin/content/news_list');
			}else{
				_message('新闻添加失败');
			}
			exit;
		}
		include $this->tpl(ROUTE_M,'news.insert');
	}
	//修改新闻
	public function news_edit(){
		$id=intval($this->segment(4));	
		$info=$this->db->GetOne("SELECT * FROM `@#_news` where `id`='$id' LIMIT 1");
		...
		if(isset($_POST['dosubmit'])){
			...
			$sql="UPDATE `@#_news` SET   `author`='$author', `title`='$title',`title_style` = '$title_style',`description`='$description', `thumb`='$thumb', `content`='$content', `posttime`='$posttime',`hit` = '$hit' WHERE (`id`='$id')";
			$this->db->Query($sql);
				
			if($this->db->affected_rows()){
				_message("修改成功!",WEB_PATH.'admin/content/news_list');
			}else{
				_message("修改失败!");
			}
			header("Cache-control: private");
		...
		include $this->tpl(ROUTE_M,'news.edit');
	}
	//新闻列表
	public function news_list(){
		$this->ment=array(
						array("lists","新闻管理",ROUTE_M.'/'.ROUTE_C."/news_list"),
						array("insert","添加新闻",ROUTE_M.'/'.ROUTE_C."/news_add"),
		);
		...
		$num=20;
		$total=$this->db->GetCount("SELECT COUNT(*) FROM `@#_news` WHERE $list_where");
		$page=System::load_sys_class('page');
		if(isset($_GET['p'])){$pagenum=$_GET['p'];}else{$pagenum=1;}	
		$page->config($total,$num,$pagenum,"0");
		$articlelist=$this->db->GetPage("SELECT * FROM `@#_news` WHERE $list_where order by `order` DESC",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));		
		
		include $this->tpl(ROUTE_M,'news.lists');
	}
	//新闻排序
	public function news_listorder(){		
		if($this->segment(4)=='dosubmit'){
			foreach($_POST['listorders'] as $id => $listorder){
				$id = intval($id);
				$listorder = intval($listorder);
				$this->db->Query("UPDATE `@#_news` SET `order` = '$listorder' where `id` = '$id'");		
			}				
			_message("排序更新成功");
		}else{
			_message("请排序");
	...
	public function news_del(){
		$id=intval($this->segment(4));		
		$this->db->Query("DELETE FROM `@#_news` WHERE (`id`='$id') LIMIT 1");
			if($this->db->affected_rows()){			
				echo WEB_PATH.'/'.ROUTE_M.'/content/news_list';
			}else{
				echo "no";
			}	
	}
...
HTML;
	...
	public function goods_add(){		
		...
		if(isset($_POST['dosubmit'])){		
			...
			$query_1 = $this->db->Query("INSERT INTO `@#_shoplist` (`zhiding`,`cateid`, `brandid`, `title`, `title_style`, `title2`, `keywords`, `description`, `money`, `yunjiage`, `zongrenshu`, `canyurenshu`,`shenyurenshu`, `qishu`,`maxqishu`,`thumb`, `picarr`, `content`,`xsjx_time`,`renqi`,`pos`,`bannershop`,`renqipos`,`newpos`,`posthumb`, `time`,`scene`,`code`) VALUES ('".htmlspecialchars($_POST['zhiding'])."','$cateid', '$brandid', '$title', '$title_style', '$title2', '$keywords', '$description', '$money', '$yunjiage', '$zongrenshu', '$canyurenshu','$shenyurenshu', '1','$maxqishu', '$thumb', '$picarr', '$content','$xsjx_time','$goods_key_renqi', '$goods_key_pos','$goods_key_bannershop','$renqipos','$newpos','$posthumb','$time','$_POST[scene]','$_POST[code]')");			
	...
	*	商品列表	
	*/
	public function general_goods_list(){
		$this->ment=array(
			array("lists","商品管理",ROUTE_M.'/'.ROUTE_C."/general_goods_list"),
			array("add","添加商品",ROUTE_M.'/'.ROUTE_C."/goods_add"),
			array("renqi","人气商品",ROUTE_M.'/'.ROUTE_C."/general_goods_list/renqi"),
			array("money","商品价格正序",ROUTE_M.'/'.ROUTE_C."/general_goods_list/money"),
			array("moneyasc","商品价格倒序",ROUTE_M.'/'.ROUTE_C."/general_goods_list/moneyasc"),
		);		
	    $cateid=$this->segment(4);
		...
		if($cateid){			
			...
			if($cateid=='money'){
				$list_where = "1 order by `money` DESC";
				$this->ment[3][1]="商品价格正序";
				$this->ment[3][2]=ROUTE_M.'/'.ROUTE_C."/general_goods_list/moneyasc";
			}
			if($cateid=='moneyasc'){
				$list_where = "1 order by `money` ASC";
				$this->ment[4][1]="商品价格倒序";
				$this->ment[4][2]=ROUTE_M.'/'.ROUTE_C."/general_goods_list/money";
			}
			if(intval($cateid)){
				$list_where = "`cateid` = '$cateid'";
			}			
		}else{
			$list_where = "1 order by `listorder` ASC,`addtime` DESC";
		}
		...
		$num=20;
		$total=$this->db->GetCount("SELECT COUNT(*) FROM `@#_shop_general` WHERE $list_where"); 
		$page=System::load_sys_class('page');
		if(isset($_GET['p'])){$pagenum=$_GET['p'];}else{$pagenum=1;}	
		$page->config($total,$num,$pagenum,"0");
		$shoplist=$this->db->GetPage("SELECT * FROM `@#_shop_general` WHERE $list_where",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));	
		include $this->tpl(ROUTE_M,'general_good.list');
	}
	
	/* 
	*	添加商品 
	*/
	public function general_goods_add(){
		if(isset($_POST['dosubmit'])){
			$cateid = intval($_POST['cateid']);
			$brandid = intval($_POST['brand']);
			$title = _htmtocode($_POST['title']);
			...
			$property = $_POST['property'] ? implode(',',$_POST['property']) : '';
			...
			$time=time();	//商品添加时间		
			$this->db->Autocommit_start();
			$query = "INSERT INTO `@#_shop_general` (`cateid`, `brandid`, `title`, `title_style`, `title2`, `keywords`, `description`, `money`, `inventory`, `thumb`, `picarr`, `content`,`renqi`,`pos`,`bannershop`,`renqipos`,`newpos`,`posthumb`, `addtime`,`edittime`,`property`) 
			VALUES ('$cateid', '$brandid', '$title', '$title_style', '$title2', '$keywords', '$description', '$money', '$inventory', '$thumb', '$picarr', '$content','$goods_key_renqi', '$goods_key_pos','$goods_key_bannershop','$renqipos','$newpos','$posthumb','$time','$time','$property')";
			$query_1 = $this->db->Query($query);
			
			if($query_1){
				$this->db->Autocommit_commit();				
				_message("商品添加成功!",WEB_PATH."/admin/content/general_goods_list");
			}else{		
				$this->db->Autocommit_rollback();
				_message("商品添加失败!");
			}
			...
			$this->ment=array(
				array("lists","商品管理",ROUTE_M.'/'.ROUTE_C."/general_goods_list"),
				array("insert","添加商品",ROUTE_M.'/'.ROUTE_C."/general_goods_add"),
			);
			include $this->tpl(ROUTE_M,'general_good.add');
		}	
	}
	...
	*	编辑商品
	*/
	public function general_goods_edit(){
		$this->db->Autocommit_start();
		$shopid=intval($this->segment(4));		
		$shopinfo=$this->db->GetOne("SELECT * FROM `@#_shop_general` WHERE `id` = '$shopid' ");
		if(!$shopinfo)_message("参数不正确");
		if(isset($_POST['dosubmit'])){
			$cateid = intval($_POST['cateid']);
			$brandid = intval($_POST['brand']);
			...
			$sql="UPDATE `@#_shop_general` SET 	`cateid` = '$cateid',
												`brandid` = '$brandid',
												`title` = '$title',
												`title_style` = '$title_style',
												`title2` = '$title2',
			...
			include $this->tpl(ROUTE_M,'general_good.edit');
		}
	...
	*	删除商品
	*/
	public function general_goods_delete(){
		$shopid=intval($this->segment(4));		
		$info = $this->db->GetOne("SELECT title FROM `@#_shop_general` WHERE `id` = '$shopid'");
		if(!$info){
			_message("删除是产品不存在!");
		}
		
		$this->db->Autocommit_start();	
		$q = $this->db->Query("DELETE FROM `@#_shop_general` WHERE `id` = '$shopid' ");
		if($q){					
			$this->db->Autocommit_commit();
			_message("删除成功！",WEB_PATH."/admin/content/general_goods_list");
		}else{
			$this->db->Autocommit_rollback();
			_message("删除失败！");
		}
	...
	*	商品排序
	*/	
	public function general_good_listorder(){
		if($this->segment(4)=='dosubmit'){
			foreach($_POST['listorders'] as $id => $listorder){
				$id = intval($id);
				$listorder = intval($listorder);
				$this->db->Query("UPDATE `@#_shop_general` SET `listorder` = '$listorder' where `id` = '$id'");		
			}				
			_message("排序更新成功");
		}else{
			_message("请排序");
	...
	*/
	public function get_category_info(){
		$catid = $this->segment(4);
		if($catid){
			$cateinfo=$this->db->GetOne("SELECT `property` FROM `@#_category` WHERE `cateid` = '$catid' LIMIT 1");
			if($cateinfo['property']){
				$property = explode('|',$cateinfo['property']);
