...
/*
	log
*/
function  _log_result($file,$word) 
{
    $fp = fopen($file,"a");
    flock($fp, LOCK_EX) ;
...
/** 
 * 生成二维码
 * @param string $qrurl 要生成二维码的链接文本
 * @param int $userid 
 * @return string $QRpath
 */
function create_qr_code($qrurl,$userid=0){
    // 加载二维码类库
    include G_APP_PATH.'addons/phpqrcode/phpqrcode.php'; 

    $errorCorrectionLevel = 'L';        //容错级别   
    $matrixPointSize = 10;              //生成图片大小 290*290
    $imgname =  $userid.'_qrcode.png' ; //生成二维码名称
    $imgpath = $imgname;
    $QRpath =G_UPLOAD.'qrcode/'.$imgname;
    // 生成二维码图片
    QRcode::png($qrurl, $imgname, $errorCorrectionLevel, $matrixPointSize, 2);   
        rename($imgpath, $QRpath);
        unlink($imgpath);
    return $QRpath ;
}

function fun_get_fx_img($openid,$uid){
	$db = System::load_sys_class("model");
	$memberinfo=$db->GetOne("SELECT * FROM `@#_member` WHERE `openid`='$openid'");
	if($memberinfo){
		
		if(substr($memberinfo['img'],0,4)=='http'){
			$userimg=$memberinfo['img'];
		}else{
			$userimg=G_UPLOAD.$memberinfo['img'];
		}
	}else{
		$userimg=G_UPLOAD.'photo/member.jpg';
	}
	$erurl=WEB_PATH."/register/".$uid;
	$qrimg=create_qr_code($erurl,$uid);
	$bgimg=G_TEMPLATES.'taolong-fangwangyi/images/beijing.jpg';
	$image= System::load_sys_class('image');
	$newqrimg=$image->thumb($qrimg,'220','220',$uid);
	$newuserimg=$image->thumb($userimg,'65','65',$uid);
	$newbgimg=$image->waterMark($bgimg,$newqrimg,10,$uid);
	$fx_img=$image->waterMark($newbgimg,$newuserimg,11,$uid);
	$fx_arr=explode('qrcode',$fx_img);
	$fx_img_path='qrcode'.$fx_arr[1];
	$db->Query("UPDATE `@#_member` SET `fx_img`='$fx_img_path' WHERE `openid`='$openid'");
	$token=gettoken();
	$type="image";
	$filedata=array("media"=>"@".$fx_img);
...
function gettoken(){
	if(file_get_contents('access_token.txt')){
		        $token=file_get_contents('access_token.txt');
	}else{
		        $db = System::load_sys_class("model");
		        $mpset=$db->GetOne("SELECT * FROM `@#_1024_wx_mpset` WHERE `id`=6");
		        $AppId = $mpset['appid'];
		        $AppSecret = $mpset['appsecret'];
		        ...
		        file_put_contents('access_token.txt',$token);
	}
