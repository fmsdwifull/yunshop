...
function upyunWapPicUpload(domid,width,height,token){
	...
	art.dialog.data('domid', domid);
	art.dialog.data('lastpic', $('#'+domid).val());
	art.dialog.open('?g=User&m=Upyun2&a=upload&from=wap&token='+token+'&width='+width,{lock:true,title:'上传图片',width:200,height:100,yesText:'关闭',background: '#000',opacity: 0.45});
}
