function upyunPicUpload(domid,width,height,token){
	...
	art.dialog.open('?g=User&m=Upyun2&a=upload&token='+token+'&width='+width,{lock:true,title:'上传图片',width:600,height:400,yesText:'关闭',background: '#000',opacity: 0.45});
}
function upyunWapPicUpload(domid,width,height,token){
	...
	art.dialog.data('domid', domid);
	art.dialog.data('lastpic', $('#'+domid).val());
	art.dialog.open('?g=User&m=Upyun2&a=upload&from=Wap&token='+token+'&width='+width,{lock:true,title:'上传图片',width:260,height:150,top:100,yesText:'关闭',background: '#000',opacity: 0.65});
}
